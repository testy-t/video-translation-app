...
export class PaymentService {
    ...
    const timerId = window.setInterval(async () => {
      ...
      try {
        ...
        if (is_paid) {
          ...
          // Отправляем цель Яндекс.Метрики при успешной оплате
          if (window.ym) {
            try {
              window.ym(101915706, 'reachGoal', 'payment-success');
              console.log("✅ Цель Яндекс.Метрики 'payment-success' отправлена");
            } catch (e) {
              console.error("❌ Ошибка отправки цели в Яндекс.Метрику:", e);
            }
          }
  ...
  /**
   * Запускает поллинг статуса активации заказа 
   * Используется на экране результата для получения ссылки на переведенное видео
   * @param uniquecode Уникальный код транзакции
   * @param onVideoReady Колбэк для получения информации о готовом видео
   * @param interval Интервал проверки в миллисекундах (по умолчанию 20000 мс - 20 секунд)
   * @param maxAttempts Максимальное количество попыток (по умолчанию 90 - около 30 минут)
   * @returns Идентификатор таймера (для возможной отмены)
   */
  static startVideoActivationPolling(
    uniquecode: string,
    onVideoReady: (videoInfo: any) => void,
    interval = 20000,
    maxAttempts = 90
  ): number {
    let attempts = 0;
    console.log(`🎬 Запуск поллинга активации видео для ${uniquecode}...`);
    
    // Сначала проверяем, вдруг видео уже готово
    this.checkPaymentStatus(uniquecode)
      .then(data => {
        if (data.is_activated && data.video && data.video.status === 'completed') {
          console.log(`✅ Видео уже активировано и готово:`, data.video);
          onVideoReady(data.video);
          return;
        } else {
          console.log(`🎬 Видео еще не готово, запускаем поллинг. Текущий статус:`, 
            data.video ? data.video.status : 'unknown');
        }
      })
      .catch(error => {
        console.error("Ошибка при первичной проверке статуса видео:", error);
      });
    ...
    const timerId = window.setInterval(async () => {
      ...
      try {
        console.log(`🔄 Проверка статуса видео (попытка ${attempts}/${maxAttempts})...`);
        const data = await this.checkPaymentStatus(uniquecode);
        
        // Проверяем статус активации и наличие готового видео
        const isVideoReady = data.is_activated && 
                             data.video && 
                             data.video.status === 'completed' && 
                             data.video.output_url;
        
        if (data.video) {
          console.log(`📊 Статус видео: ${data.video.status}, is_activated=${data.is_activated}`);
        }
        
        // Если видео готово, останавливаем поллинг и вызываем колбэк
        if (isVideoReady) {
          console.log("✅ Видео активировано и готово:", data.video);
          window.clearInterval(timerId);
          
          // Сохраняем информацию о видео в localStorage
          localStorage.setItem(`video_info_${uniquecode}`, JSON.stringify(data.video));
          
          // Вызываем колбэк с информацией о видео
          onVideoReady(data.video);
          return;
        }
        ...
        // Если достигли максимального количества попыток, останавливаем поллинг
        if (attempts >= maxAttempts) {
          console.log("⚠️ Достигнуто максимальное количество попыток проверки статуса активации");
          window.clearInterval(timerId);
        }
      } catch (error) {
        console.error("Ошибка при проверке статуса активации видео:", error);
